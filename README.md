...
For the processes described above are implemented in Python, as that is the supported scripting language for Blender, for the template creation and mesh processing, Blender is used. To load the Python dependencies, MeshProcessing.yml loaded in conda-forge will install them.
...
The 2d image is processed by FFHQ-UV on the HPC as in Section \ref{Attempt1}. Then the following files are transferred by hand to 'MeshProcessing/input/': 'L\_ball.obj', 'R\_ball.obj', 'stage3\_mesh\_id.obj', 'stage3\_mesh.mtl', 'eye\_ball\_tex.mtl' and 'stage3\_uv.png'.
